...
extern JavaVM* gJavaVM; // defined in native-lib.cpp (JNI_OnLoad)
...
void AudioEngine::cleanUpJni(JNIEnv* env) {
...
void AudioEngine::sendFrequencyToKotlin(float frequencyHz, float confidence, float rms) {
...
bool AudioEngine::start(JNIEnv* env, jobject callbackInstance) {
    if (mRunning.load(std::memory_order_acquire)) return true;

    // store global ref to callback object and method id
    mCallbackObject = env->NewGlobalRef(callbackInstance);
    jclass cbClass = env->GetObjectClass(mCallbackObject);
    mCallbackMethod = env->GetMethodID(cbClass, "onFrequencyChanged", "(FFF)V");
    ...
    // ensure aubio prepared with sampleRate from audio source; AudioSourceManager will provide stream sample rate via startIfNeeded
    // We'll try with a conservative default and rely on reconfiguration when stream opens.
    if (!prepareAubio(48000)) {
        cleanUpJni(env);
        return false;
    }

    // register to audio source manager
